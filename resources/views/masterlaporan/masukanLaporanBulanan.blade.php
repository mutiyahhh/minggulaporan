...
@section('content')
    <div class="container-fluid">
        <div class="container rounded bg-white mt-4 mb-4">
            <h3>Masukan Laporan {{ $judulLaporan->judul_laporan }} untuk Bulan: {{ $bulanLaporan['waktu_bulan_laporan'] }}
            </h3>
            ...
            <form action="{{ route('storeLaporanWithFiles') }}" method="POST" enctype="multipart/form-data">
                ...
                @if ($detailLaporan->isNotEmpty())
                    ...
                    <input type="hidden" name="waktu_tahun_laporan_id" value="{{ $year }}">
                ...
                @foreach ($detailLaporan as $detail)
                    ...
                    @if ($detail->subjudul->tipe_laporan == 'foto')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="hidden" name="subjudul_laporan_id_{{ $detail->subjudul->id }}"
                                id="subjudul_laporan_id_{{ $detail->subjudul->id }}"
                                value="{{ $detail->subjudul->id }}" />
                            <input type="hidden" name="detail_id[{{ $detail->subjudul->id }}]"
                                value="{{ $detail->id }}" />
                            <input type="file" name="photos[{{ $detail->subjudul->id }}]" class="form-control"
                                id="photos_{{ $detail->subjudul->id }}">
                        </div>
                    @elseif($detail->subjudul->tipe_laporan == 'video')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="hidden" name="subjudul_laporan_id_{{ $detail->subjudul->id }}"
                                id="subjudul_laporan_id_{{ $detail->subjudul->id }}"
                                value="{{ $detail->subjudul->id }}" />
                            <input type="text" name="videos[{{ $detail->subjudul->id }}]" class="form-control"
                                id="videos_{{ $detail->subjudul->id }}">
                        </div>
                    @endif

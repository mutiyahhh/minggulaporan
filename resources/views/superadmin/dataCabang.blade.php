...
<div class="container-fluid">
    <div class="container rounded bg-white mt-4 mb-4">
        <div class="row">
            <div class="container">
                <h3>Daftar Cabang</h3>
                <a href="{{ route('tambahCabang') }}" class="btn btn-secondary">Tambah Cabang</a>
                <form action="{{ route('cariCabang') }}" method="GET">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $cabang->area }}</td>
                            <td>{{ $cabang->alamat }}</td>
                            <td>{{ $cabang->nomor_hp }}</td>
                            <td>
                                <a href="{{ route('cabangs.show', $cabang->id) }}" class="btn btn-primary">Detail</a>
                                <a href="{{ route('cabangs.edit', $cabang->id) }}" class="btn btn-warning">Edit</a>
                                @if (auth()->user()->type == "admin")
                                    <form action="{{ route('cabangs.destroy', $cabang->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus cabang ini?')">Hapus</button>
                                    </form>
                                @endif
                            </td>
                        </tr>
                ...
                <!-- Pagination Links -->
                {{ $cabangs->withQueryString()->links() }} <!-- Pastikan cabangs adalah instance paginasi -->
            </div>
        </div>
    </div>
</div>  

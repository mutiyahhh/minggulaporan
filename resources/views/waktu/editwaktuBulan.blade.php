...
<div class="container-fluid">
    <div class="container rounded bg-white mt-4 mb-4">
        <div class="row">
            <div class="container">
                <h3>Edit Waktu Laporan Bulanan untuk Tahun {{ \Carbon\Carbon::parse($waktu_tahun->waktu_tahun_laporan)->format('Y') }}</h3>
                <form action="{{ route('waktuBulanan.update', ['tahunId' => $waktu_tahun->id, 'bulanId' => $waktu_bulan->id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <label for="waktu_bulan_laporan" class="form-label">Waktu Bulan Laporan</label>
                        <input type="month" class="form-control" id="waktu_bulan_laporan" name="waktu_bulan_laporan" value="{{ \Carbon\Carbon::parse($waktu_bulan->waktu_bulan_laporan)->format('Y-m') }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="start" class="form-label">Start</label>
                        <input type="datetime-local" class="form-control" id="start" name="start" value="{{ \Carbon\Carbon::parse($waktu_bulan->start)->format('Y-m-d\TH:i') }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="end" class="form-label">End</label>
                        <input type="datetime-local" class="form-control" id="end" name="end" value="{{ \Carbon\Carbon::parse($waktu_bulan->end)->format('Y-m-d\TH:i') }}" required>
                    </div>
                    <button type="submit" class="btn btn-primary">Update Waktu Bulan</button>
                    <a href="{{ route('waktuBulanan', $waktu_tahun->id) }}" class="btn btn-secondary">Kembali</a>
                </form>
            </div>
        </div>
    </div>

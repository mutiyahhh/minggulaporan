...
<title>Edit Waktu Tahunan</title>
...
<div class="container-fluid">        
    <div class="container rounded bg-white mt-4 mb-4">
        <div class="row">
            <div class="container">
                <h3>Edit Waktu Tahunan</h3>
                <form action="{{ route('waktuTahunan.update', $waktu_tahun->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="waktu_tahun_laporan" class="form-label">Waktu Tahun Laporan</label>
                    <select class="form-control" id="waktu_tahun_laporan" name="waktu_tahun_laporan" required>
                        <option value="">Pilih Tahun</option>
                        @for ($year = date('Y') + 1; $year >= 2019; $year--)
                            <option value="{{ $year }}" {{ $waktu_tahun->waktu_tahun_laporan == $year ? 'selected' : '' }}>{{ $year }}</option>
                        @endfor
                    </select>
                </div>
                <div class="mb-3">
                    <label for="catatan" class="form-label">Catatan</label>
                    <input type="text" class="form-control" id="catatan" name="catatan" value="{{ $waktu_tahun->catatan }}">
                </div>
                <button type="submit" class="btn btn-primary">Update Waktu Tahunan</button>
        ...
        <div class="row">
            <div class="col-md-6 order-last order-md-first">
                <div class="copyright text-md-start">
                    <p class="text-sm">
                        Developed by
                        <a href="https://www.wahanaritelindo.com/" rel="nofollow" target="_blank" class="text-red">Wahana Ritelindo</a>
                    </p>
                </div>
            </div>
            <div class="col-md-6 order-last order-md-first">
                <div class="copyright text-md-end">
                    <p class="text-sm">
                        Version <a class="text-red">1.0.0</a>
                    </p>
                </div>
            </div>
        </div>

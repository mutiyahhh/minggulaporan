...
<div class="container-fluid">
    <div class="container rounded bg-white mt-4 mb-4">
        <div class="row">
            <div class="container">
                <h3>Daftar Waktu Laporan Tahunan</h3>
                <a href="{{ route('tambahTahun') }}" class="btn btn-secondary">Tambah Waktu Tahun</a>
                <table class="table">
                    <thead>
                        <tr>
                            <th>ID</th>
                    ...
                    <tbody>
                        @foreach($waktu_tahun_laporan as $laporan)
                        <tr>
                            <td>{{ $laporan->id }}</td>
                            <td>{{ $laporan->waktu_tahun_laporan }}</td>
                            <td>{{ $laporan->catatan }}</td>
                            <td>
                                <a href="{{ route('waktuTahunan.edit', $laporan->id) }}" class="btn btn-warning">Edit</a>
                                {{-- <a href="{{ route('waktuBulanan', $laporan->id) }}" class="btn btn-info">Detail</a> --}}
                                <form action="{{ route('waktuTahunan.destroy', $laporan->id) }}" method="POST" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
